...
function blankTemplateDoc(doc: BlockDoc): BlockDoc {
  return {
    ...doc,
...
export function BlockTemplateRunner({
  ...
  const [doc, setDoc] = useState(() => blankTemplateDoc(initialDoc));
  ...
  const publicSlug = publicContext?.slug;
  const target = useMemo(
    () => publicSlug ? { kind: "user_template" as const, key: publicSlug } : null,
    [publicSlug]
  );

  useEffect(() => {
    if (target) trackView(target);
  }, [target]);
  ...
  const copy = useCallback(async () => {
    ...
    if (target) trackUse(target, "copy");
    setCopied(true);
    setToast(true);
  ...
  const providerLinks: ProviderOpenLinks = {
    chatgpt: {
      ...
      onClick: target ? () => trackUse(target, "open_chatgpt") : undefined,
    },
    claude: {
      href: openInUrl("claude", effectiveText),
      onClick: target ? () => trackUse(target, "open_claude") : undefined,
    },
    gemini: {
      href: openInUrl("gemini", effectiveText),
      onClick: target ? () => trackUse(target, "open_gemini") : undefined,
    },
  };
  ...
  return (
    <main className="builder-page tpl-dual community-block-template">
      ...
      <div className="tpl-topbar">
        ...
        {(publicContext || ownerEditHref) && (
          <div className="tpl-topbar-meta">
            {publicContext?.author && (
              <CreatorChip creator={{ kind: "community", author: publicContext.author }} />
            )}
            {target && <BookmarkButton compact target={target} />}
            {ownerEditHref && (
        ...
        <aside className="tpl-col-prompt" aria-label="Generated prompt">
          ...
          <div className="tpl-save">
            <SavePromptButton
              ...
              variant="outline"
            />
            {target && <UsesBadge target={target} />}
          </div>
        </aside>
      </div>
