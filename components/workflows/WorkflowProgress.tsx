...
export function WorkflowProgressProvider({
  ...
  const value = useMemo(
    () => ({ progress, stepIds, hydrated, setActiveStep, toggleStep, goToStep }),
    [goToStep, hydrated, progress, setActiveStep, stepIds, toggleStep]
  );
  ...
  return (
    <div className="wd-progress-summary" aria-live="polite">
      <span className="wd-progress-copy">
        <strong>{hydrated ? `${completed} of ${stepIds.length}` : "Progress"}</strong>
