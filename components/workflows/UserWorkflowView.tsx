...
import { WorkflowDisclosureControls, WorkflowStep, WorkflowStepsProvider } from "./WorkflowSteps";
...
export function UserWorkflowView({ id, title, category, overview, timeLabel, document, editHref, bookmarkSlug, remixAction }:{
  ...
  return <WorkflowStepsProvider stepIds={stepIds}><main className="workflow-detail"><div className="wd-wrap">
    <section className="wd-head"><div><span className="wd-tag">{category}</span><h1>{title}</h1><p>{overview}</p></div><aside className="wd-summary"><span><Icon name="list" size={15}/>{stepIds.length} steps</span><span><Icon name="clock" size={15}/>{timeLabel}</span>{bookmarkSlug?<BookmarkButton target={{kind:"user_workflow",key:bookmarkSlug}}/>:null}{editHref?<Link className="btn btn-ghost btn-sm" href={editHref}><Icon name="wrench" size={14}/> Edit Workflow</Link>:null}{remixAction?<form action={remixAction}><input type="hidden" name="slug" value={bookmarkSlug}/><button className="btn btn-ghost btn-sm">Remix and edit</button></form>:null}</aside></section>
    ...
    <section className="wd-steps"><WorkflowDisclosureControls/>{document.steps.map((step,index)=><WorkflowStep key={step.id} stepId={step.id} stepNumber={index+1} duration={step.duration} title={step.title} explanation={step.explanation}>
      ...
      <div className="wd-step-lists"><div><h3>Deliverables</h3><ul>{step.deliverables.filter(Boolean).map((item)=><li key={item}>{item}</li>)}</ul></div><div><h3>Tips</h3><ul>{step.tips.filter(Boolean).map((item)=><li key={item}>{item}</li>)}</ul></div></div>
    </WorkflowStep>)}</section>
  </div></main></WorkflowStepsProvider>;
}
